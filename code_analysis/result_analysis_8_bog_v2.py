...
## Get Variable data
df_bog = pd.read_csv('../data/exp_variables/bog_data_1994_2020.csv', usecols=["CIK", "bogindex", "datadate"])
df_bog = df_bog.dropna()
df_bog['datadate'] = pd.to_datetime(df_bog['datadate'], format="%m/%d/%Y")
df_bog["year"] = df_bog['datadate'].dt.year
df_bog = df_bog[["CIK", "bogindex", "year"]]

print(df_bog.shape)

df_bog['normalized_bogindex'] = (df_bog['bogindex'] - df_bog['bogindex'].mean()) / df_bog['bogindex'].std()
...
for model_name, file in [("llama_8b_3", "../data/llm_prompt_outputs_v2/llama8b_yearly_3_combined.csv"),
    ...
    ## Merge Robintrack
    df_combined_model = df_combined_model.merge(df_bog, on=['CIK', 'year'], how='inner')
    ...
    X = pd.concat([sm.add_constant(df_combined_model['normalized_bogindex']), year_dummies], axis=1)
    ...
    with open(f'./regressions_v2/result_summary_Bog_{model_name}.txt', "w") as f:
