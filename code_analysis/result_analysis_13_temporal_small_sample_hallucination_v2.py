...
gpt45_result = df_combined_gpt45.groupby('year').apply(lambda x: calculate_std_hal(x, 'gpt45')).reset_index()
...
deepseek_result = df_combined_deepseek.groupby('year').apply(lambda x: calculate_std_hal(x, 'deepseek')).reset_index()
...
# Group by 'year' and calculate the percentage of rows where 'column' equals 'low'
gemini15pro_result = df_combined_gemini15pro.groupby('year').apply(lambda x: calculate_std_hal(x, 'gemini15pro')).reset_index()
...
# Group by 'year' and calculate the percentage of rows where 'column' equals 'low'
finma_result = df_combined_finma.groupby('year').apply(lambda x: calculate_std_hal(x, 'finma')).reset_index()
...
# Plotting
plt.plot(combined_result.year, combined_result.gpt45_hallucination, color='m', label='GPT-4.5')
plt.fill_between(combined_result.year, (combined_result.gpt45_hallucination-combined_result.gpt45_std), \
                                        (combined_result.gpt45_hallucination+combined_result.gpt45_std), color='m', alpha=0.1)

plt.plot(combined_result.year, combined_result.deepseek_hallucination, color='b', label='DeepSeek V3')
plt.fill_between(combined_result.year, (combined_result.deepseek_hallucination-combined_result.deepseek_std), \
                                        (combined_result.deepseek_hallucination+combined_result.deepseek_std), color='b', alpha=0.1)

plt.plot(combined_result.year, combined_result.gemini15pro_hallucination, color='darkorange', label='Gemini 1.5 Pro')
plt.fill_between(combined_result.year, (combined_result.gemini15pro_hallucination-combined_result.gemini15pro_std), \
                                        (combined_result.gemini15pro_hallucination+combined_result.gemini15pro_std), color='darkorange', alpha=0.1)

plt.plot(combined_result.year, combined_result.finma_hallucination, color='r', label='FinMA-7B-full')
plt.fill_between(combined_result.year, (combined_result.finma_hallucination-combined_result.finma_std), \
                                        (combined_result.finma_hallucination+combined_result.finma_std), color='r', alpha=0.1)
...
plt.title('Hallucination Rate For Different Models (Small Sample)')
